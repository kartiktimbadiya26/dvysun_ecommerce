                        ...
                        <div class="table-responsive">
                              <table class="table table-hover align-middle table-nowrap mb-0">
                                    ...
                                    <tbody>
                                          ...
                                          $query = "SELECT reviews.id, reviews.name , reviews.email , reviews.title , reviews.message , reviews.rate , reviews.createdate ,product.name as pro_name  FROM reviews JOIN product ON reviews.product_id=product.id LIMIT $limit OFFSET $skip";
                                          $result = mysqli_query($conn, $query);
                                          $temp = 1;
                                          if (isset($_GET["page"])) {
                                                if ($page != 1)
                                                ...
                                                <tr>
                                                      ...
                                                      <td class="text-center">
                                                            <div class="d-flex flex-nowrap justify-content-center">
                                                                  <form method="post">
                                                                        <!-- <a href="update-reviews.php?id=<?php echo $row["id"]; ?>"
            ...
            <nav aria-label="Page navigation example w-100 overflow-scroll " class="mt-6 mb-4">
                  <ul class="pagination justify-content-start overflow-scroll">
                        <li class=" page-item mx-3">
                              <a class="page-link" href="reviews.php?page=<?php
                              if ($page > 1)
                                    echo $page - 1;
                              else
                        ...
                        for ($i = 1; $i <= ceil($count / $limit); $i++) {
                              if ($page == $i) { ?>
                                    <li class="page-item active mx-3">
                                          <a class="page-link"
                                                href="reviews.php?page=<?php echo $i; ?>&limit=<?php echo $limit; ?>">
                                                <?php echo $i; ?>
                                          </a>
                                    </li>
                              <?php } else { ?>
